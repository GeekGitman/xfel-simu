...
def compare(fname, s1,  s1_name, outpath):
    ...
    POS = fname.find('lstc')
    output_name = s1_name + '_' + fname[POS:-3] + '.txt'
...
#    filelist = ['lstc0.h5']
    ...
    fname_std = "./h5lines/lstc0.h5"
    s1_name = 'g3_0'
    h1 = h5py.File(fname_std,'r')
    s1 = h1['data'].value
    # computing
    if isParallel:
        par_compare = partial(compare, s1=s1, s1_name=s1_name,outpath=outpath)
